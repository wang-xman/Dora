...
/*
    ...
    a (Ta) / b (Tb) : Objects of which the difference is asserted.
        Compare the Euclidean norm of the difference bwteen a and
        b with the given err_margin. If it is smaller than err_margin,
        asserts true; otherwise throws assertion error.
    
    err_margin : A number of double type. It defines the error margin.
*/
template <typename Ta, typename Tb>
void assert_approx_equal(int line, const Ta& a, const Tb& b, double err_margin)
{
    if (sqrt( std::norm(a - b) ) > err_margin) {
        std::string errmsg = "Assert approx equal failed.";
        throw AssertionError(errmsg, line);
    }
}
...
/*
    ...
    E : Exception class of which an error is expected to be thrown.
    ...
    line (int) : Line number to which this assert function is invoked.

    function : Function which is to be invoked inside assertion function.
...
/*
    ...
    E : Exception class in which the error shall be thrown.
    ...
    line (int) : Line number to the line where assertion function is invoked.
    
    obj : An instance of class C.
    
    method : Pointer to the method to be invoked in assertion function.
    ...
    EXAMPLE
        ...
        TestClass obj;
        assert_method_throw<ErrorClass>(
